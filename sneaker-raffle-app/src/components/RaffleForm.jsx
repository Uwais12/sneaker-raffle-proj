...
function RaffleForm({ sneakers, raffles = [] }) {
  ...
  const sneakerOptions = sneakers.map((sneakerOption) => ({ value: sneakerOption.id, label: `${sneakerOption.id} | ${sneakerOption.name} | ${sneakerOption.brand}` }));
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 space-y-4 w-full">
      ...
      <Creatable
        value={region}
        onChange={setRegion}
        options={Array.from(new Set(raffles.map((raffle) => raffle.region)))
          .map((regionOption) => ({ value: regionOption, label: regionOption }))}
        className="mb-2 w-full"
      />
